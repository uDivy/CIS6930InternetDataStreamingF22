...
s = int(input("Number of Cuckoo Steps(s):\n"))

print("The value of n, m, k and s are:",n, m, k, s)
...
"""
This will insert the new flowid into the hash table, but if all the position for 
current flowid resulted into hash collision, it will call move() function
"""
...
"""
the function will try to move the occupied flow entry into it's another hash location in it's 
first cuckoo step and following step been recursively followed if we use more than 1 cuckoo steps. 
"""
def move(x, s):
    flowid_to_rmv = hash_table[x][0]
    for i in range(k):
        h = multiplication_hash(n, flowid_to_rmv ^ randomnums[i])
        if h != x and hash_table[h][0] == 0:
            hash_table[x] = (0, 0)
            hash_table[h] = (flowid_to_rmv, hash_table[h][1])
            return True

    if s > 1:
        for i in range(k):
            # here we are finding the address of the flowid which is been occupied by the other location of f1..
            h = multiplication_hash(n, flowid_to_rmv ^ randomnums[i])
            if h != x and move(h, s - 1):
                hash_table[h] = (flowid_to_rmv, hash_table[h][1])
                return True
